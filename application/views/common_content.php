   ...
   <div class="page-head xs-pt-10 xs-pb-10">
      <div class="row">
         ...
         <div class="col-md-10">
            ...
            <button class="btn btn-default dropdown-toggle" data-toggle="dropdown" type="button" aria-expanded="false"><span id="judul"><?php echo lang($province_current) ?></span>&nbsp;<span class="caret"></span></button>
               <ul class="dropdown-menu" role="menu" id="dLabeli">
                  <?php foreach ($province_access as $key => $value): ?>
                     <li><a href="<?php echo site_url($segmen).'/'.$value['id'] ?>" onclick="link(this.href); return false"><?php echo lang($value['name']) ?></a></li>
                  <?php endforeach ?>
               </ul>
            </div>
         </div>
...
<div class="row" style="padding:12px 12px 0px 12px;" id="Sfr_Cont_IdBoxInfoDataGrid" style="display: none;">
   <div class="col-md-12">
      <div id="Sfr_IdBoxInfoDataGrid" class="Sfr_BoxInfoDataGrid" style="display:none;">
      </div>
   </div>
</div>

<?php if (isset($viewAdditional)) : ?>
   <div id='row-fluid'>
<?php endif; ?>
<?php

   if (isset($viewAdditional)) {
         echo $this->load->view($viewAdditional, $action, true);
   }
?>
