...
class Mars extends SS_Controller {
	...
	public function index($prov='',$private='',$kab='') {		
        $template       = 'dashboard/mars';
        $data['js']     = 'dashboard/mars';
        $this->action['partner'] = 9; // Mars
		$data['action'] = array_merge(
			array(
                'prov'      => $prov,
                'kab'       => $kab,
                'priv'      => $private,
                'data'      => $this->api.'/dashboard/mars',
                'cert_holder' => $this->input->get('cert_holder')?$this->input->get('cert_holder'):'all',
			)
			,$this->action);
