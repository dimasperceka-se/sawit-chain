...
class Changepassword extends SS_Controller {

    public function __construct() {
        parent::__construct(1);
        $this->lang->load('changepassword');
        $this->api = $this->config->item('api');
        $url = (index_page())?(base_url().index_page()):rtrim(base_url(),'/');
        $this->titlet = lang('Change Password').' > ';
    }
    ...
    public function index() {
        ...
        $data['action'] = array('data'=>$this->api.'/user/mypassword');

        $data['titlet'] = lang('Change Password');
