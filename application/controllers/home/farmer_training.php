...
class Farmer_training extends SS_Controller {
    ...
    public function index($prov='',$private='',$kab='') {
        ...
        $data['js'] = 'dashboard/farmer_training';
        $data['action'] = array_merge(
            array(
                ...
                'priv'=>$private,
                'regen' => $_GET['regen'],
                'data'=>$this->api.'/dboard/dash_get_farmer_training',
                'now'=>date('Y')
            ), $this->action);
        $data['title'] = $this->titlet.lang('Farmer Training');
        $data['mentokDistrict'] = true;
        $this->LoadView($data, 'dashboard/farmer_training');
    }
