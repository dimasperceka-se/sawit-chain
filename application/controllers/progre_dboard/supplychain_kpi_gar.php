...
class Supplychain_Kpi_Gar extends SS_Controller
{
    ...
    public function index($prov = '', $kab = '')
    {
        ...
        $data['js']    = 'progre_dboard/supplychain_kpi_gar';
        $data['action'] = array_merge(
            array(
                ...
                'petani' => $_GET['petani'],
                'tahun' => $data['tahun'],
                'data'  => $this->api . '/dboard/dash_get_supplychain_kpi_gar'),
                $this->action
        );
        $data['title'] = $this->titlet . lang('Program Supply Chain KPI (GAR)');
        $this->LoadView($data, 'progre_dboard/supplychain_kpi_gar');
    }
