...
class Supplychain_Mill_Kpi_Full extends SS_Controller
{
    ...
    public function index($prov = '', $kab = '')
    {
        ...
        $data['js']    = 'progre_dboard/supplychain_mill_kpi_full';
        ...
        $data['action'] = array_merge(
            array(
                ...
                'petani' => $_GET['petani'],
                'tahun' => $data['tahun'],
                'data'  => $this->api . '/dboard/dash_get_supplychain_mill_kpi_full'),
                $this->action
        );
        $data['title'] = $this->titlet . lang('Program Supply Chain KPI');
        $this->LoadView($data, 'progre_dboard/supplychain_mill_kpi_full');
    }
