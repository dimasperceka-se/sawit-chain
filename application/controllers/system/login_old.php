...
class Login extends SS_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('login_v2', $data);
    }

    public function tor_view(){
        $data = array();
    ...
    , i.FlagAccess
    , IF(st.ObjType = 'private' || st.ObjType = 'program',st.ObjID,NULL) AS PartnerID
...
LEFT JOIN ktv_private_staff d ON d.PersonID = p.PersonID AND d.StatusCode = 'active'
LEFT JOIN ktv_program_staff f ON f.PersonID = p.PersonID AND f.StatusCd = 'active'
...
-- LEFT JOIN ktv_program_partner i ON i.PartnerID = d.PartnerID
LEFT JOIN (
    SELECT
