...
class Kpi_sawit_terampil extends SS_Controller
{
    ...
    public function index($wave= '', $chdistrict='', $lockdate='') {
        ...
        $data['js'] = 'dboard/kpi_sawit_terampil';
        ...
        $data['action'] = array_merge(
            array(
                'data'      => $this->api.'/dboard/kpi_sawit_terampil',
                'wave' => $wave, 
                'chdistrict' => $chdistrict,
                'lockdate'=>$lockdate,
            ), $this->action
        );
        $data['title'] = $this->titlet.lang('KPI Sawit Terampil');
        $this->LoadView($data, 'dboard/kpi_sawit_terampil');
    }
}
?>