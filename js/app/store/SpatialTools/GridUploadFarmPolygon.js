 Ext.define('Koltiva.store.SpatialTools.GridUploadFarmPolygon', {
    extend: 'Ext.data.Store',
    storeId: 'Koltiva.store.SpatialTools.GridUploadFarmPolygon',
    id: 'Koltiva.store.SpatialTools.GridUploadFarmPolygon',
    fields: ['MemberDisplayID','MemberName','PlotNr','SurveyNr','Lat','Lng','Polygon','AreaHa','Valid','Remark'],
    autoLoad: true,    
    proxy: {
        type: 'ajax',
        url: m_api + '/map/farm_polygon_client',
        params: {
            'X-API-KEY': '030584'
        },
        reader: {
    ...
    listeners: {        
        load: function(store) {        
            ...
            Ext.getCmp('Koltiva.view.SpatialTools.MainView').clearMap()
            ...
            $.each(tmp_data, function( i, {data} ) {
                ...
                if(Polygon){
                    ...
                    Ext.getCmp('Koltiva.view.SpatialTools.MainView').addPolygon(tag, id, area, attr, color)
                }
            })
            ...
            if(store.getTotalCount()>0){
                ...
                Ext.getCmp('Koltiva.view.SpatialTools.MainView').addMarker(dataPoint)
