...
*/
Ext.define('Koltiva.store.GrowerWAGS.GridMain', {
    extend: 'Ext.data.Store',
    id: 'store.Grower.GridMain',
    fields: ['MemberIDInc','id', 'Name','MillName','Desa','Kecamatan','LastUpdated','Province','District','Birthdate','Age','DateCollection','Handphone','MaritalStatus', 'MemberRole','Enumerator','DateCreated','NrOfPlantation','TotalHectare','TotalHectarePolygon','PartnerSurvey','isCertified','SupplybaseType'],
    pageSize: 30,
    autoLoad: false,
    remoteSort: true,
    proxy: {
    ...
    listeners: {
            ...
            if(success == true){
                Ext.Ajax.request({
                    ...
                    success: function(data) {
                        Ext.getCmp('view.GrowerWAGS.GridMainGrower-gridInformation').update(data.responseText);
                    }
                });
            }
        },
        ...
        beforeload: function(store, operation, options){
            ...
            if(patchouli_grower_adv_ls != null){
                ...
                pAdvLastUpdatedDateEnd = patchouli_grower_adv_ls.pAdvLastUpdatedDateEnd;
            }else{
                pAdvRowEnumerator = "";
                pAdvTextEnumerator = "";
                pAdvRowHandphone = "";
            ...
            store.proxy.extraParams.textSearch = Ext.getCmp('view.GrowerWAGS.GridMainGrower-textSearch').getValue();
            store.proxy.extraParams.textSearchDesa = Ext.getCmp('view.GrowerWAGS.GridMainGrower-textSearchDesa').getValue();
            store.proxy.extraParams.categorySearch = Ext.getCmp('view.GrowerWAGS.GridMainGrower-CmbCategorySearch').getValue();
