		...
		marker: {
	        ...
	        , events: {
	            click: function (marker, event, context) {
                    ...
                    getSupplyProfile(context.data.id, context.data.type);
	            }
	        }
		}
	});
...
function getSupplyProfile(id, type) {
    if(type=='do'){
        var type_link = 'agent';
    }else{
        var type_link = type;
    }
    var url_profile = url_api+'/geospatial/supply_profile_'+type_link+'?id='+id+'&start='+supplychain.start+'&end='+supplychain.end+'&partner='+supplychain.partner;
    ...
    var url_transaction = url_api+'/geospatial/supply_transaction_'+type+'?id='+id+'&start='+supplychain.start+'&end='+supplychain.end+'&partner='+supplychain.partner;
    $.get(url_transaction, function(data) {
        var bruto = 0;
        var netto = 0;
        trans = '<table width="100%" class="table table-condensed table-hover table-bordered table-striped"><thead><tr><th>No</th><th>Date</th>'+(type != 'farmer'?'<th>Batch</th>':'<th>Number</th>')+'<th>'+lang('Bruto')+' (Kg)</th><th>'+lang('Netto')+' (Kg)</th>'+(type != 'warehouse'?('<th>Dest</th>'):'')+'</tr></thead><tbody>';
        $.each(data, function(index, val) {
            trans += '<tr><td>'+(index+1)+'</td><td>'+val.trans_date+'</td>'+(type != 'farmer'?('<td>'+val.batch_number+'</td>'):('<td>'+val.trans_number+'</td>'))+'<td style="text-align:right">'+number_format(val.bruto,2,'.',',')+'</td><td style="text-align:right">'+number_format(val.netto,2,'.',',')+'</td>'+(type != 'warehouse'?('<td>'+val.dest_orgname+'</td>'):'')+'</tr>';
            bruto += parseFloat(val.bruto);
            netto += parseFloat(val.netto);
        });
        trans += '<tr><td colspan="3">Total (in Kg)</td><td style="text-align:right"><strong>'+number_format(bruto,2,'.',',')+'</strong></td><td style="text-align:right"><strong>'+number_format(netto,2,'.',',')+'</strong></td><td></td></tr>';
        trans += '<tr><td colspan="3">Total (in '+lang('Ton')+ ')</td><td style="text-align:right"><strong>'+number_format(bruto/1000,3,'.',',')+'</strong></td><td style="text-align:right"><strong>'+number_format(netto/1000,3,'.',',')+'</strong></td><td></td></tr>';
        trans += '</tbody></table>';
