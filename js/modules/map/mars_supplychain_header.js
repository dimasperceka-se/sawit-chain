...
Supplychain.prototype.showDetail = function(id){
    ...
    console.log(point.detail_fetched);
                ...
                mouseover: function (marker, event, context) {
                    ...
                    if (context.data.level < 5) {
                        if (context.data.detail_shown === true) {
                            profile += '<tr><td colspan="3" style="text-align:center;"><a class="green_btn" href="#" style="cursor:pointer;" onclick="supplychain.showProfile(' + context.data.id + ', ' + "'" + context.data.type + "'" +')">'+lang('Show Profile')+'</a> <a class="green_btn" style="cursor:pointer;" onclick="supplychain.hideDetail('+context.data.id+')">'+lang('Hide Detail')+'</a></td></tr>';
                        } else {
                            profile += '<tr><td colspan="3" style="text-align:center;"><a class="green_btn" href="#" style="cursor:pointer;" onclick="supplychain.showProfile(' + context.data.id + ', ' + "'" + context.data.type + "'" +')">'+lang('Show Profile')+'</a> <a class="green_btn" style="cursor:pointer;" onclick="supplychain.showDetail('+context.data.id+')">'+lang('Show Details')+'</a></td></tr>';
                        }
                    }
                    profile += '</tbody></table>';
...
Supplychain.prototype.renderPoly = function(){
    ...
    function curvedLine(options) { 
        ...
        for(var i = 0; i < Points.length; i++) {
            ...
            curvedLineCreateSegment(LastLat,LastLng,PartLat, PartLng, o.Color, o.Opacity, o.Weight, o.GapWidth, o.Map, o.data);
            
            LastLat = PartLat;
            LastLng = PartLng;
        }
        
        curvedLineCreateSegment(LastLat, LastLng, o.LatEnd, o.LngEnd, o.Color, o.Opacity, o.Weight, o.GapWidth, o.Map, o.data);
    }
    
    function curvedLineCreateSegment(LatStart, LngStart, LatEnd, LngEnd, Color, Opacity, Weight, GapWidth, Map, data) {
...
function getSupplyProfile(id, type, parent) {
    ...
    $.get(url_profile, function(data) {
        ...
        if (type!='warehouse') profile += '<tr><td>' + lang('Delivered to Supplier') + '</td><td>:&nbsp;</td><td>' + data.delivered_count + '</td></tr>';
