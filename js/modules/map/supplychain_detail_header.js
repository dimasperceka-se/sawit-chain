...
Supplychain.prototype.removePointByParent = function(parent_id, exclude_id, type){
    // console.log(parent_id);
    for (var i = this.points.length - 1; i >= 0; i--) {
        if (this.points[i].parent == parent_id && this.points[i].id != exclude_id && this.points[i].id != parent_id) {
            if (type && this.points[i].type != type) {
                continue;
            }
            this.removePolyById(this.points[i].id);
            this.points[i].hidden = true;
...
Supplychain.prototype.showProfile = function(id, tipe){
    if(id=='tba'){
        ...
        if(tipe=='warehouse'){
            preview_cetak_surat(m_cetak_mill_profile+'MillID/'+id);
        }else if(tipe=='farmer'){
            preview_cetak_surat(m_cetak_beneficiary_profiles+'MemberID/'+id);
        }else{
            preview_cetak_surat(m_cetak_agent_profile+'MemberID/'+id);
        }
    }
};
...
Supplychain.prototype.showDetail = function(id){
    ...
    if (point.detail_fetched == false && supplychain.warehouse!=id) {
        get_farmer(id, point, this.partner, this.certification, supplychain.warehouse, function(){
...
Supplychain.prototype.showFarmer = function(id){    
    var thisObj = this;
    var point = this.getPointById(id);
    // var certification = this.certification;
    // this.removePointByLevel(point.level, point.id);
    if (point.farmer_fetched == false) {
        get_farmer(id, point, this.partner, this.certification, supplychain.warehouse, function(data){
            if (data) {
                thisObj.setPointAttr(id, 'farmer_shown', true);
                supplychain.reDrawMap(false);
                supplychain.setPointAttr(id, 'farmer_fetched', true);
            } else {
                Ext.Msg.alert('Info', lang('Farmer not found'));
            }
        });
    } else {
        this.setPointAttr(id, 'farmer_shown', true);
        this.restorePoint('parent', id);
        this.reDrawMap(false);
    }
};

Supplychain.prototype.showRefinery = function(id){    
    var thisObj = this;
    var point = this.getPointById(id);
    // var certification = this.certification;
    // this.removePointByLevel(point.level, point.id);
    if (point.refinery_fetched == false) {
        get_refinery(id, point, this.partner, this.certification, supplychain.warehouse, function(data){
            if (data) {
                thisObj.setPointAttr(id, 'refinery_shown', true);
                supplychain.reDrawMap(false);
                supplychain.setPointAttr(id, 'refinery_fetched', true);
            } else {
                Ext.Msg.alert('Info', lang('Refinery not found'));
            }
        });
    } else {
        this.setPointAttr(id, 'refinery_shown', true);
        this.restorePoint('parent', id);
        this.reDrawMap(false);
    }
};
...
Supplychain.prototype.hideFarmer = function(id){
    var point = this.getPointById(id);
    this.removePointByParent(id, null, 'farmer');
    this.setPointAttr(id, 'farmer_shown', false);
    this.restorePointByParent(point.parent);
    // this.restorePointByLevel(point.level);
    this.reDrawMap(false);
};

Supplychain.prototype.hideRefinery = function(id){
    var point = this.getPointById(id);
    this.removePointByParent(id, null, 'refinery');
    this.setPointAttr(id, 'refinery_shown', false);
    this.restorePointByParent(point.parent);
    // this.restorePointByLevel(point.level);
    this.reDrawMap(false);
};
                ...
                click: function (marker, event, context) {
                    ...
                    if (context.data.level < 5) {
                        if (context.data.detail_shown === true) {
                            profile += '<tr><td colspan="3" style="text-align:center;">';
                            profile += '<a class="green_btn" href="#" style="cursor:pointer;" onclick="supplychain.showProfile(' + context.data.id + ', ' + "'" + context.data.type + "'" +')">'+lang('Show Profile')+'</a> ';
                            if (context.data.type != 'trader') {
                                profile += '<a class="green_btn" style="cursor:pointer;" onclick="supplychain.hideDetail('+context.data.id+')">'+lang('Hide Detail')+'</a> ';
                            }
                            if (context.data.farmer_shown === true) {
                                profile += '<a class="green_btn" style="cursor:pointer;" onclick="supplychain.hideFarmer('+context.data.id+')">'+lang('Hide Farmer')+'</a> ';
                            } else {
                                profile += '<a class="green_btn" style="cursor:pointer;" onclick="supplychain.showFarmer('+context.data.id+')">'+lang('Show Farmer')+'</a> ';    
                            }
                            if (context.data.type == 'warehouse') {
                                if (context.data.refinery_shown === true) {
                                    profile += '<a class="green_btn" style="cursor:pointer;" onclick="supplychain.hideRefinery('+context.data.id+')">'+lang('Hide Refinery')+'</a>';
                                } else {
                                    profile += '<a class="green_btn" style="cursor:pointer;" onclick="supplychain.showRefinery('+context.data.id+')">'+lang('Show Refinery')+'</a>';    
                                }
                            }
                            profile += '</td></tr>';
                        } else {
                    ...
                    getSupplyProfile(context.data.id, context.data.type, context.data.parent);
                }
            }
        }
    });
...
function get_refinery(id, to, partner, certification, warehouse, callback){
    var start       = $date_start.val();
    var end         = $date_end.val();
    $.ajax({
        url: url_supplychain_refinery,
        data: {
            id : id,
            warehouse : warehouse,
            partner: partner,
    ...
    .done(function(data) {
        $.each(data, function(index, val) {
            if (Math.abs(parseFloat(val.latitude)) > 0 && Math.abs(parseFloat(val.latitude)) < 90) {
                ...
                refinery.label   = lang('Refinery');
                refinery.name    = val.name;
                refinery.lat     = parseFloat(val.latitude);
                refinery.lng     = parseFloat(val.longitude);
                refinery.netto                = parseFloat(val.netto);
                refinery.transaction_count    = parseFloat(val.reception_count);
                supplychain.addPoint(refinery);

                var refinery_poly = {};
                refinery_poly.id          = val.id;
                refinery_poly.type        = 'refinery';
                refinery_poly.label       = lang('Refinery');
                refinery_poly.from        = val.name;
                refinery_poly.to          = to.name;
                refinery_poly.from_type   = 'refinery';
                refinery_poly.to_type     = '';
                refinery_poly.bruto                = '';
                refinery_poly.netto                = val.netto;
                refinery_poly.transaction_count    = val.reception_count;
                refinery_poly.supply_count         = '';
                refinery_poly.batch_count          = '';
                refinery_poly.path = [
                    [parseFloat(val.latitude), parseFloat(val.longitude)],
                    [parseFloat(to.lat), parseFloat(to.lng)]
                ];
                supplychain.addPoly(refinery_poly);
            }
        });
        // supplychain.reDrawMap(false);
        callback(data);
...
function get_farmer (id, to, partner, certification, warehouse, callback) {
    var start       = $date_start.val();
    var end         = $date_end.val();
    $.ajax({
...
function getSupplyProfile(id, type, parent) {
    var url_profile = url_api+'/geospatial_new/supply_profile_'+type+'?id='+id+'&start='+supplychain.start+'&end='+supplychain.end+'&partner='+supplychain.partner+'&certification='+supplychain.certification+'&warehouse='+supplychain.warehouse+'&parent='+parent;
    $.get(url_profile, function(data) {
        profile = '<table border="0" width="100%"><tbody>';
        if (type=='farmer') profile += '<tr><td>' + lang('PlotNr') + '</td><td>:&nbsp;</td><td>' + data.PlotNr + '</td></tr>';
        ...
        if (type=='farmer') profile += '<tr><td>' + lang('Delivered to Supplier') + '</td><td>:&nbsp;</td><td>' + data.delivered_count + '</td></tr>';
        ...
        if (type!='refinery') profile += '<tr><td>' + lang('Bruto') + '</td><td>:&nbsp;</td><td>' + number_format(data.bruto/1000,3,'.',',') +' '+lang('Ton')+ '</td></tr>';
        profile += '<tr><td>' + lang('Netto') + '</td><td>:&nbsp;</td><td>' + number_format(data.netto/1000,3,'.',',') +' '+lang('Ton')+ '</td></tr>';
        if (type=='farmer') profile += '<tr><td colspan="3"><center><a style="line-height: 14px;" class="green_btn" onclick="supplychain.showProfile(' + "'" + data.member_id + "'" + ', ' + "'" + data.member_type + "'" +')" href="#"> ' + lang('Show Profile') + ' </a></center></td></tr>';
        
        profile += '</tbody></table>';
        $('#supply_profile').prepend(profile);
    });
    var url_transaction = url_api+'/geospatial_new/supply_transaction_'+type+'?id='+id+'&start='+supplychain.start+'&end='+supplychain.end+'&partner='+supplychain.partner+'&certification='+supplychain.certification+'&warehouse='+supplychain.warehouse+'&parent='+parent;
    $.get(url_transaction, function(data) {
        var bruto = 0;
        var netto = 0;
        var exports = '';
        if(type == 'refinery'){
            trans = '<table width="100%" class="table table-condensed table-hover table-bordered table-striped"><thead><tr><th>No</th><th>Date</th><th>'+lang('Netto')+' (Kg)</th><th>Dest</th><th>Status</th>'+'</tr></thead><tbody>';
            $.each(data, function(index, val) {
                trans += '<tr><td>'+(index+1)+'</td><td>'+val.trans_date+'</td><td style="text-align:right">'+number_format(val.netto,2,'.',',')+'</td>'+(type != 'warehouse'?('<td>'+val.dest_orgname+'</td>'):'')+'<td>'+val.batch_status+'</td>'+'</tr>';
            ...
            if(netto > 0){
                exports = '<table width="100%"><tbody><tr><td><a class="green_btn" style="cursor:pointer;" href="'+url_api+'/geospatial_new/export_supply_transaction_'+type+'?id='+id+'&start='+supplychain.start+'&end='+supplychain.end+'&partner='+supplychain.partner+'&certification='+supplychain.certification+'&warehouse='+supplychain.warehouse+'&parent='+parent+'" target="_link">'+lang('Export')+'</a></td></tr></tbody></table>';
            }
        }else{            
            trans = '<table style="padding:20px 10px 20px 10px;" width="100%" class="table table-condensed table-hover table-bordered table-striped"><thead><tr><th>No</th><th>Date</th>'+(type != 'farmer'?'<th>Registration ID</th>':'<th>Registration ID</th>')+'<th>'+lang('Bruto')+' (Kg)</th><th>'+lang('Netto')+' (Kg)</th>'+(type != 'warehouse'?('<th>Dest</th>'):'')+(type == 'warehouse'?('<th>From</th>'):'')+'<th>Status</th>'+'</tr></thead><tbody>';
            $.each(data, function(index, val) {
        ...
        $('#supply_transaction').html(exports+trans);
    });
}
