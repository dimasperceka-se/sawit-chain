...
Ext.onReady(function(){
    ...
    var store = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: m_api + '/basic_staff/main_list',
            params: {
                'X-API-KEY': '030584'
            },
            reader: {
...
function displayFormStaff(displayMethod,cmb_objtype,store,StaffID,viewOnly) {
    ...
    var cmb_mill = Ext.create('Ext.data.Store', {
        extend: 'Ext.data.Model',
        fields: ['id', 'label'],
        autoLoad: true,
        proxy: {
    ...
    var cmb_refinery = Ext.create('Ext.data.Store', {
        extend: 'Ext.data.Model',
        fields: ['id', 'label'],
        autoLoad: true,
        proxy: {
    ...
    var cmb_sme = Ext.create('Ext.data.Store', {
        extend: 'Ext.data.Model',
        fields: ['id', 'label'],
        autoLoad: false,
        proxy: {
    ...
    var storeGridMainRelation = Ext.create('Ext.data.Store', {
        extend: 'Ext.data.Model',
        ...
        listeners: {
            'beforeload': function(store, options) {
                ...
                store.proxy.extraParams.key = Ext.getCmp('view.basic.staff.relation-textSearch').getValue();
                store.proxy.extraParams.StaffID = StaffID;
            }
        }
    });

    var storeGridMain = Ext.create('Ext.data.Store', {
        model: 'mainGridModel.Model',
        autoLoad: true,
        pageSize: 50,
        remoteSort: true,
    ...
    function setFilterLs() {
        localStorage.setItem('patchouli_grower_ls', JSON.stringify({
            key: Ext.getCmp('view.basic.staff.relation-textSearch').getValue()
        }));
    }
    ...
    function submitOnEnterGridGrower(field, event) {
        if (event.getKey() == event.ENTER) {
            setFilterLs();
            Ext.getCmp('view.basic.staff.relation-gridMainGrid').getStore().loadPage(1);
        }
    }    
    ...
    var contextMenuGridRelation = Ext.create('Ext.menu.Menu', {
        items: [{
            icon: varjs.config.base_url + 'images/icons/new/update.png',
            text: lang('Update'),
            handler: function () {
                var sm = Ext.getCmp('view.basic.staff.relation-gridMainGrid').getSelectionModel().getSelection()[0];

                displayFormStaffRelation('update',StaffID,true,sm.get('StaffRelID'));
            }
        },{
            icon: varjs.config.base_url + 'images/icons/new/delete.png',
            text: lang('Delete'),
                    ...
                    if (btn == 'yes') {
                        Ext.Ajax.request({
                            ...
                            success: function (response, opts) {
                                ...
                                //refresh store
                                setFilterLs();
                                Ext.getCmp('view.basic.staff.relation-gridMainGrid').getStore().load();
                            },
                            failure: function (response, opts) {
                                var pesanNya;
    ...
    function displayFormStaffRelation(displayMethod,StaffID,viewOnly,StaffRelID) {
        ...
        var winFormStaffRelation = Ext.create('widget.window', {
            title: lang('Form Staff Relation'),
    ...
    var winFormStaff = Ext.create('widget.window', {
        ...
        items:[{
            ...
            items:[{
                ...
                xtype:'panel',
                title: 'C. '+lang('Traceability Staff Work Relation'),
                frame:true,
                bodyStyle:{"background-color":"#F0F0F0"},
                style:'background-color:#F0F0F0;',
                padding:2,
                items:[{
                    ...
                    items: [{
                        ...
                        items:[{
                            ...
                            items:[{
                                xtype: 'grid',
                                id: 'view.basic.staff.relation-gridMainGrid',
                                style: 'border:1px solid #CCC;margin-top:4px;',
                                loadMask: true,
                                minHeight: 320,
                                selType: 'rowmodel',
                                store: storeGridMainRelation,
                                viewConfig: {
                                    deferEmptyText: false,
                                    emptyText: lang('Empty Data')
                                },
                                dockedItems: [{
                                    xtype: 'pagingtoolbar',
                                    id: 'view.basic.staff.relation-gridToolbar',
                                    store: storeGridMainRelation,
                                    dock: 'bottom',
                                    displayInfo: true
                                }, {
                                    ...
                                    items: [{
                                            ...
                                            text: lang('Add'),
                                            handler: function () {
                                                displayFormStaffRelation('add',StaffID,true,'');
                                            }
                                        },
                                        {
                                            name: 'key',
                                            baseCls: 'Sfr_TxtfieldSearchGrid',
                                            id: 'view.basic.staff.relation-textSearch',
                                            xtype: 'textfield',
                                            width: 400,
                                ...
                                columns: [{
                                    ...
                                    items: [{
                                        ...
                                        handler: function (grid, rowIndex, colIndex, item, e, record) {
                                            contextMenuGridRelation.showAt(e.getXY());
                                        }
                                    }]
                                }, {
...
function displayFormUser(StaffID,store){
    ...
    var user_projects = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: m_api + '/basic_staff/user_project',
            reader: {
                type: 'json',
                root: 'data'
            }
    ...
    var access_role = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: m_api + '/basic_staff/access_role',
            reader: {
                type: 'json',
                root: 'data'
            }
    ...
    Ext.getCmp('winFormDataUser').getForm().load({
        url: m_api + '/basic_staff/form_user',
        method: 'GET',
        params: {
            StaffID: StaffID
        },
        success: function(form, action) {
            ...
            if(r.data.UserId == null){
                ...
                Ext.getCmp('winBtnDelete').setVisible(true);
            }
        },
        failure: function(form, action) {
            Ext.MessageBox.show({
    ...
    function set_selector_access_object(UserId = null) {
        ...
        if(fieldListAccessRole.length > 0){
            ...
            $.each(fieldListAccessRole, function(index, val) {
                ...
                Ext.Ajax.request({
                    url: m_api + '/basic_staff/access_object_list',
                    method: 'POST',
                    async: false,
                    timeout: 3600,
                    params: {
...
function displayFormUserApp(StaffID,store){
    ...
    var store_app_access_staff = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: m_api + '/basic_staff/list_access_staff_app',
            params: {
                'X-API-KEY': '030584'
            },
            reader: {
            ...
            handler: function () {
                ...
                if (form.isValid()) {
                    if(Ext.getCmp('AppGroupUid').getValue()[0] == null || Ext.getCmp('AppRoleUid').getValue()[0] == null){
                        ...
                        form.submit({
                            url: m_api + '/basic_staff/user_app',
                            method:'POST',
                            waitMsg: 'Saving data...',
                            success: function(fp, o) {
                                var jsonResp = o.result;
    ...
    Ext.getCmp('winFormDataUserApp').getForm().load({
        url: m_api + '/basic_staff/form_user_app',
        method: 'GET',
        params: {
            StaffID: StaffID
        },
