Ext.onReady(function(){
    var store = Ext.create('Ext.data.Store', {
        ...
        fields: ['SaleID','OrgID','OrgType','Date','SaleNumber','Number','CustomerID','Name','Total','Pembayaran'],
        autoLoad: true,
        pageSize: 50,
        proxy: {
            type: 'ajax',
    ...
    function set_update(sm) {
       ...
       Ext.Ajax.request({
           ...
           success: function(fp, o){
               ...
               Ext.getCmp('name_sale').setValue(sm.get('SaleNumber'));
    ...
    var mc_customer = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: m_api+'bussiness/penjualan_customer',
            reader: {
                type: 'json',
                root: 'data'
            }
   ...
   Ext.define("Post_sale", {
        extend: 'Ext.data.Model',
        proxy: {
            type: 'ajax',
            url : m_crud+'_penjualan',
            reader: {
                type: 'json',
                root: 'data',
                totalProperty: 'total'
        ...
        fields: [
            {name: 'SaleID', mapping: 'SaleID'},
            {name: 'CustomerID', mapping: 'CustomerID'},
            {name: 'Total', mapping: 'Total'},
            {name: 'Number', mapping: 'Number'}
        ]
    });
    var mc_sale = Ext.create('Ext.data.Store', {
        pageSize: 10,
        model: 'Post_sale'
    });
    //end store combo
    ...
    var DataForm = Ext.create('Ext.form.Panel', {
        ...
        items: [{
                 ...
                 items: [{
                   ...
                   listeners: {
                     ...
                     change: function (cb, nv, ov) {
                        if (Ext.getCmp('OrgType').getValue()!=null) {
                           ...
                           mc_sale.proxy.extraParams.OrgType = Ext.getCmp('OrgType').getValue();
                           mc_sale.proxy.extraParams.OrgID = Ext.getCmp('OrgID').getValue();
                        }
                     }
                   }
               },{
                  fieldLabel: lang('Sale'),
                  xtype: 'combo',
                  store: mc_sale,
                  id:'name_sale',
                  name:'SaleNumber',
                  ...
                  listeners: {
                      select: function(combo, selection) {
                          ...
                          if (post) {
                              Ext.getCmp('ReturSaleID').setValue(post.get('SaleID'))
                              Ext.getCmp('CustomerID').setValue(post.get('CustomerID'))
                              store_detail.load({
                                   params: {
                                       'ReturSaleID': post.get('SaleID')
                                   }
                              });                              
                          }
                      }
           ...
           listeners: {
               ...
               'edit': function (editor, e) {console.log(e.record.data);
