...
var ajaxDataRenderer = function(url) {
   ...
   $.ajax({
        type: "GET",
        url: url,
        data: {prov: m_prov,kab: m_kab,kec: m_kec, desa: m_desa, priv: m_priv,daer: m_daer,awal: m_awal,akhir: m_akhir,partner:m_partner,traceability_partner:m_traceability_partner,mill:m_mill,do:m_do,agent:m_agent},
        ...
        success: function(r) {
            ...
            var data_total_farmer = r['total_farmer'];
            var s2      = [];
            var s211 = [];
            var s212 = [];
            var s321    = [];
            ...
            for (var i=0;i<data_total_farmer.length;i++) {
                box25 += parseInt(data_total[i]['total_farmer_sell']);
             }
            ...
            $('#box_sales').text(number_format(box_sales/1000,2,'.',','));
            $('#box_cert_sales').text(number_format(box_cert_sales/1000,2,'.',','));
            $('#box_farmer_sales').text(number_format(box_farmer_sales,0,'.',','));
...
//column(s.chart_sales, 'chart_sales', lang('Traceable Sales Detail (MT)'), '', ['#95130b','#FFBC65'], s.cat_sales, 'total', 0, true);
column(s.chart_farmer, 'chart_farmer', lang('Number of Farmer Sales Detail'), '', ['#95130b','#FFBC65'], s.cat_sales, 'total', 0, true);

column_one(s.chart_certified_farmer, 'chart_certified_farmer', lang('Certified Producers vs Certified Sales'), '', ['#95130b','#FFBC65'], s.cat_traceability_farmer, 'normal', 0, true);
column_one(s.chart_uncertified_farmer, 'chart_uncertified_farmer', lang('Non Certified Producers vs Non Certified sales'), '', ['#95130b','#FFBC65'], s.cat_traceability_farmer, 'normal', 0, true);
column_one(s.chart_certified_production, 'chart_certified_production', lang('Certified Production vs Certified Sales'), '', ['#95130b','#FFBC65'], s.cat_traceability_production, 'normal', 0, true);
column_one(s.chart_uncertified_production, 'chart_uncertified_production', lang('Non Certified Production vs Non Certified Sales'), '', ['#95130b','#FFBC65'], s.cat_traceability_production, 'normal', 0, true);
