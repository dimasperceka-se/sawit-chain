...
class CognitoClient
{
    ...
    public function listUsersWithFilter($Filter)
    {
        try {
    ...
    public function adminRegisterUser($username, $password, $MessageAction, array $attributes = []) {
        $userAttributes = $this->buildAttributesArray($attributes);
        ...
        try {
            $response = $this->client->adminCreateUser($arrCreateUser);
            return $response;
            //return $response['UserSub'];
        } catch (CognitoIdentityProviderException $e) {
            //throw CognitoResponseException::createFromCognitoException($e);
    ...
    public function adminSetUserPassword($username,$password) {
        $arrSetPass = array(
            'Password' => $password,
            'Permanent' => true,
            'UserPoolId' => $this->userPoolId,
        ...
        try {
            return $this->client->adminSetUserPassword($arrSetPass);
            //return $response['UserSub'];
        } catch (CognitoIdentityProviderException $e) {
            //throw CognitoResponseException::createFromCognitoException($e);
            return $e->getMessage();
    ...
    //confirm reset dari forgot password
    public function confirmForgotPassword($username,$newPassword,$confirmCode) {
        $arrParam = array(
            'ClientId' => $this->appClientId,
        ...
        try {
            return $this->client->confirmForgotPassword($arrParam);
        } catch (CognitoIdentityProviderException $e) {
            return $e->getMessage();
            //throw CognitoResponseException::createFromCognitoException($e);
        }
    ...
    public function decodeAccessToken($accessToken)
    {
        ...
        if (!$jwsVerifier->verifyWithKeySet($jws, $keySet, 0)) {
            ...
            //echo json_encode(array('success' => false, 'message' => 'could not verify token or config aws failed ! '));
            //exit; //exit di matikan agar bisa lanjut pengecekan manual
