 ...
 */
class Mbasic_staff_sta extends CI_Model {
        ...
        //if($_SESSION['is_admin'] == "1"){
            ...
            (
                ...
                FROM
		            ...
		            LEFT JOIN ktv_mill mill on mill.MillID = s.ObjID
                WHERE
                    ((s.`ObjType` = ?) OR ('' = ?)) AND
                    s.StatusCode != 'nullified' AND
                    IF(s.ObjType <> 'MILL',s.ObjID IN ( 159, 179, 78 ),mill.PartnerID IN (159,179,78)) AND
                    ps.PersonNm LIKE ?
            ) AS tbl_for_sort
            GROUP BY StaffID
    ...
    public function updateFoto($gambar,$userId){
        $sql = "UPDATE  `koltiva_cocoatrace`.`ktv_persons` SET  `Photo` =  ? WHERE  `ktv_persons`.`UserID` = ?";
