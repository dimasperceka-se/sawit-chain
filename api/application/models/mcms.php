...
class Mcms extends CI_Model
{
    ...
    public function GetGridMainAnnouncement($start, $limit){        
    ...
    public function GetAnnouncementFormOpen($AnnID){
        $sql = "SELECT
                    a.AnnID
                    , a.`Title`
                    , a.`Content`
    ...
    public function InsertAnnouncement($ParamPost){
        ...
        $sql = "INSERT INTO `cms_announcement` SET  
                `Title` = ?,
                `Content` = ?,
                `StatusType` = ?,  
                `CreatedBy` = ?,
        ...
        $ProsesAccess = $this->InputAccess('insert','announcement',$AnnID,$ParamPost);
    ...
    public function UpdateAnnouncement($ParamPost){
        ...
        $sql = "UPDATE `cms_announcement` a SET
                    a.Title = ?,
                    a.Content = ?,
                    a.`StatusType` = ?,
                    a.`DateUpdated` = NOW(),
        ...
        $ProsesAccess = $this->InputAccess('update','announcement',$ParamPost['AnnID'],$ParamPost);        
    ...
    public function CleanUpInactiveVideoData(){
        ...
        for ($i=0; $i < count($data); $i++) {
            //hapus attachmentnya
            if(file_exists('images/video/'.$data[$i]['PicThumb'])){
                unlink('images/video/'.$data[$i]['PicThumb']);
            }
        }
    ...
    public function CleanUpInactiveNewsData(){
        ...
        for ($i=0; $i < count($data); $i++) {
			//hapus attachmentnya
            if(file_exists('uploads/news/'.$data[$i]['PhotoFile'])){
                unlink('uploads/news/'.$data[$i]['PhotoFile']);
            }
        }
        ...
        if(isset($DataList[0]['NewsID'])){
            for ($i=0; $i < count($DataList); $i++) {
                ...
                if($DataList[$i]['PhotoFile'] != "" && file_exists('uploads/news/'.$DataList[$i]['PhotoFile'])){
                    $ImgImages = '<img style="max-width:300px" id="img_'.$DataList[0]['NewsID'].'" src="'.base_url().'/uploads/news/'.$DataList[$i]['PhotoFile'].'" />';
                }else{
                    $ImgImages = '';
                }
    ...
    public function CleanUpInactiveDocumentData(){
        ...
        for ($i=0; $i < count($data); $i++) {
            //hapus attachmentnya
            if(file_exists('files/cms_document/'.$data[$i]['DocUrl'])){         
                @unlink('files/cms_document/'.$data[$i]['DocUrl']);
            }
        }
