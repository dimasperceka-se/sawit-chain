...
class m_trans_batch extends CI_Model
{
    ...
    public function get_data_trans($SID, $PID)
    {
        ...
        */
        //    $sql = 'SELECT 
        //                 b.*, 
        //                 sum(a.VolumeNetto) AS VolumeNetto, 
        //                 sum(a.VolumeBruto) AS VolumeBruto
        //            FROM 
        //                 ktv_tc_supplychain_transaction a
        //             LEFT JOIN 
        //                 ktv_tc_supplychain_batch b ON a.SupplyBatchID = b.SupplyBatchID
        //             WHERE
        //                 b.StatusCode = "active"
        //             AND 
        //                 b.SupplyOrgID = ?
        //             ORDER BY 
        //                 b.SupplyBatchID DESC';
        $sql = 'SELECT  
                    sb.*,
                    IF (SUM(st.VolumeBruto) IS NULL, FORMAT(0, 2), SUM(st.VolumeBruto)) AS VolumeBruto,
                    IF (SUM(st.VolumeNetto) IS NULL, FORMAT(0, 2), SUM(st.VolumeNetto)) AS VolumeNetto
                FROM 
                    `ktv_tc_supplychain_batch` AS sb
                LEFT JOIN 
                    `view_tc_supplychain_org` vso ON `vso`.`SupplychainID` = `sb`.`SupplyOrgID`
                LEFT JOIN 
                    `view_tc_supplychain_org` vso1 ON `vso1`.`SupplychainID` = `sb`.`SupplyDestMillOrgID`
                LEFT JOIN 
                    `ktv_tc_supplychain_transaction` st ON `st`.`SupplyBatchID` = `sb`.`SupplyBatchID`
                WHERE 
                    `sb`.`SupplyBatchStatus`IN ("Open","Closed")
                AND 
        ...
        if ($Q->num_rows()) {
            ...
            foreach ($result as $key => $val) {
                ...
                $dt_trans = $this->db->select('*')
                    ->from('ktv_tc_supplychain_transaction c')
                    ->join('ktv_tc_supplychain_batch d', 'd.SupplyBatchID = c.SupplyBatchID','left')
                    ->where('c.StatusCode', 'active')
                    ->where('c.SupplyBatchID', $val->SupplyBatchID)
                    ->get();
