...
class Mjurnal extends CI_Model {
    ...
    public function getData($start = 0,$limit = 20,$sort = 'journalDate',$dir = 'DESC', $query = array()) {
        ...
        $this->db->where('accounting_journal.CoopID', $this->coop,false);
    ...
    public function getById($id) {
        $this->db->select(array('JournalID','JournalTypeCode','JournalDate','JournalMemo'));
        $this->db->from('accounting_journal');
    ...
    public function get_coa_code($start_date,$end_date,$coa)
    {
        ...
        if($coa)
        {
                $query .= "accounting_coa.coaCode = '".$coa."' ";
        }
        ...
        //             ORDER BY journalClosedDate DESC
        //             LIMIT 0,1)"; 
         // $query .= " accounting_coa_balance.journalClosedID "; 
    ...
    public function get_coa_balance($closing_date, $start_date,$coa){
        ...
        $query = "SELECT 	
                        accounting_coa.coaCode,accounting_coa_balance.coaBalanceAmount,
                        SUM(DEBIT) AS TOTAL_DEBIT,
                        SUM(CREDIT) AS TOTAL_CREDIT
                FROM accounting_coa
                LEFT JOIN accounting_coa_balance ON accounting_coa_balance.coaCode=accounting_coa.coaCode
                WHERE accounting_coa.coaType=1 
                AND journalIsPosted=1 AND
                DATE_FORMAT(journalDate,'%Y-%m-%d') > '".$closing_date."' AND 
                DATE_FORMAT(journalDate,'%Y-%m-%d') < '".$start_date."'  AND
                accounting_coa.coaCode IN (SELECT accounting_coa.coaCode as coaCode
                            FROM accounting_coa
                                    LEFT JOIN accounting_coa_balance ON accounting_coa_balance.coaCode=accounting_coa.coaCode
                                    LEFT JOIN accounting_journal_closed ON accounting_journal_closed.journalClosedID=accounting_coa_balance.journalClosedID
                                    WHERE ".$sub."
                                    accounting_coa_balance.journalClosedID = (SELECT journalClosedID
                                                                                ...
                                                                                ORDER BY journalClosedDate DESC
                                                                                LIMIT 0,1))
                GROUP BY accounting_coa.coaCode
                ORDER BY accounting_coa.coaCode ASC, DATE_FORMAT(journalDate, '%Y-%m-%d') ASC";
        $query = $this->db->query($query);
    ...
    public function getCombo($table,$value,$display,$query = false,$val = false) {
        ...
        if(mysql_real_escape_string($query) != ''){
            ...
            $this->db->or_where($value . ' REGEXP "' . $query . '"');
        }
    ...
    public function add($data) {
        $this->db->insert('accounting_journal',array(
            'JournalTypeCode' => $data['journalTypeCode'],
            'JournalDate' => date('Y-m-d',  strtotime($data['journalDate'])),
            'JournalMemo' => $data['journalMemo'],
            'JournalCRBY' => $data['JournalCRBY'],
            'CoopID' => $data['CoopID'],
            'JournalCRDT' => date('Y-m-d H:i:s')
        ));
        $ji = $this->db->insert_id();
        
        $detail = json_decode($data['data'],TRUE);
        
        if($ji && count($detail) > 0){
            foreach($detail as $keys => $values){
                
                $this->db->insert('accounting_journal_detail',array(
                    'JournalID' => $ji,
                    'coaCode' => $values['coaCode'],
                    'JournalDetailDesc' => $values['journalDetailDesc'],
                    'currencyID' => $values['currencyID'],
                    'CoopID' => $data['CoopID'],
                    'JournalDetailOrig' => $values['journalDetailOrig'],
                    'JournalDetailExRate' => $values['journalDetailExRate'],
                    'JournalDetailSum' => $values['journalDetailOrig'] * $values['journalDetailExRate'],
                    'JournalDetailType' => ($values['DEBET'] > 0 && $values['KREDIT'] == 0)?1:2,
                    
                ));
                
                if($this->db->_error_number()){
                    if($this->db->_error_number() == 1062){
                        return array('success' => false, 'msg' => 'JOURNAL already exist');
                    }

                    return array('success' => false, 'msg' => $this->db->_error_message());
                }
            }
        }
    ...
    public function edit($id,$data) {
        ...
        $this->db->where('JournalID',$id);
        $this->db->update('accounting_journal',array(
            'JournalTypeCode' => $data['JournalTypeCode'],
            'JournalDate' => date('Y-m-d',  strtotime($data['JournalDate'])),
            'JournalMemo' => $data['JournalMemo'],
            'JournalUPBY' => $data['JournalUPBY'],
            'JournalUPDT' => date('Y-m-d H:i:s')
        ));
        
        //delete detail
        $this->db->delete('accounting_journal_detail',array('JournalID' => $id));
        //re-insert detail
        
        $detail = json_decode($data['data'],TRUE);
        if($id && count($detail) > 0){
            foreach($detail as $keys => $values){
                $this->db->insert('accounting_journal_detail',array(
                    'JournalID' => $id,
                    'CoaCode' => $values['coaCode'],
                    ...
                    'JournalDetailSum' => $values['journalDetailOrig'] * $values['journalDetailExRate'],
                    'JournalDetailType' => ($values['DEBET'] > 0 && $values['KREDIT'] == 0)?1:2,
                    'JournalDetailUPBY' => $data['JournalUPBY'],
                    'JournalDetailUPDT' => date('Y-m-d H:i:s'),
                ));
            }
        }
        if($this->db->_error_number()){
    ...
    public function delete($id) {
        //delete related data
        $this->db->where('journalID',$id);
        $this->db->delete('accounting_journal_detail');
