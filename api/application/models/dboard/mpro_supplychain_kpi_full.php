...
//                echo "</pre>";
    ...
    /**
     * Sumber data real-time supply chain KPI langsung dari tabel transaksi
     * (ktv_members, ktv_survey_plot, ktv_survey_plot_sme, ktv_survey_plot_status_mill,
     * ktv_mill, dst) tanpa lewat cache dash_pro_supplychain_kpi_full.
     *
     * Menghasilkan kolom yang identik dengan tabel dash_pro_supplychain_kpi_full,
     * di-group per SubDistrictID untuk satu PartnerID (sesuai hak akses display),
     * jadi bisa langsung dipakai sebagai derived table pengganti tabel cache.
     */
    private function realtimeSupplyChainKpiSource($PartnerID) {
        $p = $this->db->escape_str($PartnerID);
        ...
        return "
            ...
            FROM
                (
                    SELECT kv.SubDistrictID AS SubDistrictID
                    FROM ktv_members a
                        JOIN ktv_member_role r ON a.MemberID = r.MemberID
                        INNER JOIN ktv_access_partner_member acc_pm ON a.MemberID = acc_pm.apmMemberID AND acc_pm.apmPartnerID = '$p'
                        LEFT JOIN ktv_village kv ON kv.VillageID = a.VillageID
                    WHERE a.StatusCode = 'active'
                        AND a.VillageID IS NOT NULL AND a.VillageID != '' AND a.VillageID != '0'
                    GROUP BY kv.SubDistrictID
                    UNION
                    SELECT kv.SubDistrictID AS SubDistrictID
                    FROM ktv_mill a
                        INNER JOIN ktv_access_partner_mill acc_pml ON a.MillID = acc_pml.apmiMillID AND acc_pml.apmiPartnerID = '$p'
                        LEFT JOIN ktv_village kv ON kv.VillageID = a.VillageID
                    WHERE a.StatusCode = 'active'
                        AND a.VillageID IS NOT NULL AND a.VillageID != '' AND a.VillageID != '0'
                    GROUP BY kv.SubDistrictID
                ...
                LEFT JOIN (
                    ...
                    WHERE a.StatusCode = 'active'
                    GROUP BY kv.SubDistrictID
                ) AS tbl_farmer ON tbl_farmer.SubDistrictID = base.SubDistrictID
                ...
                LEFT JOIN (
                    ...
                    FROM
                        ktv_survey_plot a
                        JOIN (SELECT p.MemberID, p.PlotNr, MAX(p.SurveyNr) AS SurveyNr
                            FROM ktv_survey_plot p WHERE p.StatusCode = 'active'
                            GROUP BY p.MemberID, p.PlotNr) AS gar_latest
                            ON a.MemberID = gar_latest.MemberID AND a.PlotNr = gar_latest.PlotNr AND a.SurveyNr = gar_latest.SurveyNr
                        JOIN ktv_members m ON m.MemberID = a.MemberID
                        LEFT JOIN ktv_village kv ON kv.VillageID = m.VillageID
                        INNER JOIN ktv_access_partner_member acc_pm ON m.MemberID = acc_pm.apmMemberID AND acc_pm.apmPartnerID = '$p'
                    WHERE a.StatusCode = 'active' AND m.StatusCode = 'active'
                    GROUP BY kv.SubDistrictID
                ) AS tbl_garden_farmer ON tbl_garden_farmer.SubDistrictID = base.SubDistrictID
                ...
                LEFT JOIN (
                    ...
                    WHERE a.StatusCode = 'active' AND m.StatusCode = 'active'
                    GROUP BY kv.SubDistrictID
                ) AS tbl_garden_sme ON tbl_garden_sme.SubDistrictID = base.SubDistrictID
                ...
                LEFT JOIN (
                    ...
                    FROM
                        ...
                        JOIN ktv_mill m ON m.MillID = a.MillID
                        LEFT JOIN ktv_village kv ON kv.VillageID = m.VillageID
                        INNER JOIN ktv_access_partner_mill acc_pm ON m.MillID = acc_pm.apmiMillID AND acc_pm.apmiPartnerID = '$p'
                    WHERE a.StatusCode = 'active' AND m.StatusCode = 'active'
                    GROUP BY kv.SubDistrictID
                ) AS tbl_garden_mill ON tbl_garden_mill.SubDistrictID = base.SubDistrictID
                ...
                LEFT JOIN (
                    ...
                    GROUP BY tgrup_agent.SubDistrictID
                ) AS tbl_agent ON tbl_agent.SubDistrictID = base.SubDistrictID
                ...
                LEFT JOIN (
                    ...
                    WHERE a.StatusCode = 'active'
                        AND a.NDAAgree = 1
                    GROUP BY kv.SubDistrictID
                ) AS tbl_mill ON tbl_mill.SubDistrictID = base.SubDistrictID
        ";
    }
    ...
    public function getDisplaySupplyChainKpi($ProvinceID, $DistrictID) {
        ...
        if ($_SESSION['is_admin'] == "1") {
            ...
            $filterPartner = $_SESSION['PartnerID'];
        } else {
            //cek ktv_access_staff
            $sqlHakAkses = " AND kd.DistrictID IN (" . $_SESSION['daerah_access'] . ")";
            $sqlHakAkses .= " AND a.PartnerID = '1' #Partner Koltiva ";
        ...
        //sumber real-time langsung dari tabel transaksi (bukan cache dash_pro_supplychain_kpi_full)
        $realtimeSource = $this->realtimeSupplyChainKpiSource($filterPartner);
