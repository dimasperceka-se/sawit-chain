...
class m_supplychain_org_rel extends CI_Model {
    ...
    public function get_data($SupplychainID, $recordStart = 0, $recordLimit = 12, $sortingField = '', $sortingDir = ''){
        ...
        $sql_total = "SELECT count(RelID) AS total from ktv_tc_supplychain_org_rel where ChildID = ? ";
        ...
        try{
            ...
            if($data['RelID'] != '' ){
				...
				if($cek == 0){
					...
					$this->db->where(array('ParentID'=>$data['ParentID'], 'ChildID' => $data['ChildID']));
