 ...
 */
class Mfarmer_apps_new extends CI_Model {
    ...
    function getFarmerTransaction($data) {
        ...
        CASE
                ...
                WHEN a.PaymentStatusID = 1 THEN
                'Paid' 
                WHEN a.PaymentStatusID = 2 THEN
                'Waiting Payment' 
                WHEN a.PaymentStatusID = 3 THEN
                'Incomplete Payment' 
                WHEN a.PaymentStatusID = 5 THEN
                'To be confirm' 
                WHEN a.PaymentStatusID = 99 THEN
                'Failed' ELSE 'Not yet paid' 
...
//            die();
    ...
    function getFarmerManualNew($data) {
        $appname = @$data['attributes']['appname'];
        $farmerid = @$data['attributes']['farmerid'];
        $partnerid = @$data['attributes']['partnerid'];
        $traderid = @$data['attributes']['traderid'];
        ...
        if(isset($languagecode)){
            if(($languagecode == 'id' || $languagecode == 'in')){
                $language = 'indonesia';
                ...
                $language = 'vietnamese';
            } else if($languagecode == 'phi'){
                $language = 'philippine';
            } else if($languagecode == 'ms' || $languagecode == 'my'){
                $language = 'malay';
            } else {
                $language = 'english';
            }
        ...
        if ($id != '') {
            ...
            foreach($manual->result() as $key => $res){
                ...
                $manual->result()[$key]->Title = $this->getContentLanguageValue('Document', $res->id, 'Name', ucwords($language));
                $manual->result()[$key]->Summary = $this->getContentLanguageValue('Document', $res->id, 'Summary', ucwords($language));
                $manual->result()[$key]->Description = $this->getContentLanguageValue('Document', $res->id, 'Description', ucwords($language));
                if(!empty($manual->result()[$key]->Content)){
                    $manual->result()[$key]->Content = $this->config->item('CTCDN')."/".$manual->result()[$key]->Content;
                }
            }

            $resultManual = $manual->result();

            foreach ($resultManual as $key => $value) {
                if (empty($value->Title) && empty($value->Summary) && empty($value->Description)) {
                    unset($resultManual[$key]);
                }
            }

            if (count($resultManual) > 0) {
                $resultManual = array_values($resultManual);

                $i = 0;
                $data_convert = convert_language($resultManual, $language);
                foreach ($data_convert as $row) {
                    $ret[$i] = array(
                        'type' => "manual",
                        'id' => $row->id,
    ...
    function getNewsNew($data) {
        //https://demo.cocoatrace.com/api/index.php/farmer-apps/news
        $appname = @$data['attributes']['appname'];
        $farmerid = @$data['attributes']['farmerid'];
        $partnerid = @$data['attributes']['partnerid'];
        $traderid = @$data['attributes']['traderid'];
        ...
        if ($id != '') {
            ...
            foreach($news->result() as $key => $res){
                ...
                $news->result()[$key]->Description = $newsContent;
                $news->result()[$key]->Content     = $res->ImagePath;
            }

            $resultNews = $news->result();

            foreach ($resultNews as $key => $value) {
                if (empty($value->Title) && empty($value->Summary) && empty($value->Description)) {
                    unset($resultNews[$key]);
                }
            }

            if (count($resultNews) > 0) {
                $resultNews = array_values($resultNews);
                // get news attachment, entah kenapa loog ini tidak bisa masuk ke loop dibawah

                /* $result = $news->result_array();
                $news_attachment = [];
                foreach ($result as $key => $value) {
                    $attachment = $this->getNewsAttachment($value['NewsID']);
                    if ($attachment) {
                        $news_attachment[$value['NewsID']] = $attachment;
                    }
                } */

                $i = 0;
                $data_convert = nonconvert_language($resultNews, $language);//$news->result();
                foreach ($data_convert as $row) {
                    $ret[$i] = array(
                        'type' => "news",
                        'id' => $row->NewsID,
                    ...
                    $attachment = $this->getNewsAttachmentNew($row->NewsID['value']);
                    if ($attachment) {
                        $attach = [];
                        foreach ($attachment as $key => $value) {
                            $attach[] = $value['FileURL'];
                        }
                        $ret[$i]['attributes']['attachment'] = nonconvert_language((object)$attach, $language);
                    }
    ...
    private function getNewsAttachmentNew($NewsID)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($data as $key => $value) {
                ...
                $data[$key]['FileURL'] = $this->awsfileupload->showFileURL($type, AWSS3_NEWS_IMAGE, $value['Filename']);
            }
            return $data;
        }
        return false;
    ...
    function getVideoNew($data) {
        ...
        if ($id != '') {
            ...
            $videoNews = $video->result();
            foreach ($videoNews as $key => $value) {
                if (empty($value->Title) && empty($value->Summary) && empty($value->Description)) {
                    unset($videoNews[$key]);
                }
            }
            ...
            if (count($videoNews) > 0) {
                $videoNews = array_values($videoNews);

                $i = 0;
                $data_convert = convert_language($videoNews, $language);
                foreach ($data_convert as $row) {
                    $ret[$i] = array(
                        'type' => "video",
                        'id' => $row->id,
