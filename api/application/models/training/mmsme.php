...
class Mmsme extends CI_Model {
    ...
    function readDatas($key, $prov, $dist, $subdist, $start, $limit, $userid) {
        ...
        $sql = "
            SELECT SQL_CALC_FOUND_ROWS
                a.MSMETrainingID as id,CpgTrainings as training,BatchNumber as batch,g.`PartnerName` as partner_name,
                District as tot,(SELECT COUNT(DISTINCT aa.PartStaffID) FROM ktv_msme_trainings_participants aa WHERE aa.MSMETrainingID=a.MSMETrainingID AND aa.StatusCode='active') as participant,date(TrainingStart) as start,date(TrainingEnd) as end,
                TrainingDays as days
            from ktv_msme_trainings a
            left join ktv_cpg_trainings b on a.CPGtrainingsID=b.CpgTrainingsID
            left join ktv_cpg_batch c on a.CpgBatchID=c.CpgBatchID
            #left join ktv_msme_trainings_participants d on a.MSMETrainingID=d.MSMETrainingID
            ...
            AND (f.DistrictID IN (
               ...
               $wh)
               $where
            GROUP BY a.MSMETrainingID ORDER BY a.MSMETrainingID LIMIT ?,?";
        /*$query = $this->db->query(sprintf($sql, 'a.MSMETrainingID as id,CpgTrainings as training,BatchNumber as batch,g.`PartnerName` as partner_name,
            District as tot,count(distinct PartStaffID) as participant,date(TrainingStart) as start,date(TrainingEnd) as end,
            TrainingDays as days', 'GROUP BY a.MSMETrainingID ORDER BY a.MSMETrainingID LIMIT ?,?'), array("%$key%", $prov, (int) $start, (int) $limit, $userid));*/
    ...
    function readData($id) {
        $sql = "
            ...
            LEFT JOIN ktv_msme_trainings_sub_topics c ON a.MSMETrainingID = c.MSMETrainingID
            WHERE a.MSMETrainingID=?";
    ...
    function createData($training, $staf, $start, $end, $days, $batch, $tot, $staf_mitra, $provinsi, $district, $service_provider_id, $service_provider_staff, $userid,$CpgTrainingsIDSubTopic,$TrainingDayStatus) {
        $sql = "
            INSERT INTO ktv_msme_trainings (CPGtrainingsID, FacProgramStaffID, TrainingStart, TrainingEnd,
        ...
        if ($stat) {
            $MSMETrainingID = $this->db->insert_id();

            //sub topic (begin)
            if($CpgTrainingsIDSubTopic[0] != ""){
                foreach ($CpgTrainingsIDSubTopic as $key => $value) {
                    $sql="INSERT INTO `ktv_msme_trainings_sub_topics` SET
                          `MSMETrainingID` = ?,
                          `SubCpgTrainingsID` = ?,
                          `DateCreated` = NOW(),
                          `CreatedBy` = ?";
    ...
    function updateData($training, $staf, $start, $end, $days, $batch, $provinsi, $district, $tot, $mitra, $service_provider_id, $service_provider_staff, $id, $userid,$CpgTrainingsIDSubTopic,$TrainingDayStatus) {
        ...
        if ($stat) {
            //sub topics (begin)
            $sql="DELETE FROM ktv_msme_trainings_sub_topics WHERE MSMETrainingID = ?";
            $query = $this->db->query($sql,array($id));
    ...
    function deleteData($id) {
        /*$sql = "DELETE FROM ktv_msme_trainings WHERE MSMETrainingID=?";
        $sql_participant = "DELETE FROM ktv_msme_trainings_participants WHERE MSMETrainingID=?";
        $stat = $this->db->query($sql_participant, array($id));
        $stat = $this->db->query($sql, array($id));*/
        $sql = "UPDATE ktv_msme_trainings SET StatusCode = 'nullified',LastModifiedBy='".$_SESSION['userid']."',DateUpdated=NOW() WHERE MSMETrainingID = ? LIMIT 1";
        $stat = $this->db->query($sql, array($id));
        if ($stat) {
            $results['success'] = true;
    ...
    function readParticipants($id, $start, $limit) {
        $sql = "
            SELECT SQL_CALC_FOUND_ROWS
                MSMETrainingPartID as participant_id,
            ...
            FROM ktv_msme_trainings_participants a
            ...
            WHERE MSMETrainingID=? AND a.StatusCode != 'nullified'
...
WHERE
    ...
    AND kcf.FarmerID NOT IN (
        SELECT
            p.PartFarmerID
        FROM ktv_msme_trainings_participants p
        WHERE
            PartFarmerID IS NOT NULL AND p.MSMETrainingID = ?
    )
    --where--
ORDER BY `name`
LIMIT ?, ?
...
WHERE
    ...
    AND StaffID NOT IN (
        SELECT
            tp.PartStaffID
        FROM ktv_msme_trainings_participants tp
        WHERE
            tp.MSMETrainingID = ?
    )
    --where--
...
LIMIT ?, ?
    ...
    function createParticipants($MSMETrainingID, $type, $participants, $userid) {

        $record = array();
        $this->db->trans_start(FALSE);
        foreach ($participants as $participant) {
            $data = array(
                'MSMETrainingID'         => $MSMETrainingID,
                'PartType'              => $type,
                'WritingAwal'           => 0,
                'WritingAkhir'          => 0,
                'BallotAwal'            => 0,
            ...
            // $record[] = $data;
            $this->db->insert('ktv_msme_trainings_participants', $data);
        }
    ...
    function createParticipant($training, $staf, $wstart, $wend, $bstart, $bend, $userid) {
        $sql = "
            insert into ktv_msme_trainings_participants (MSMETrainingID, ParticipantPersonID,ParticipantNewStaffID,
    ...
    function deleteParticipant($id) {
        //$sql = "DELETE FROM ktv_msme_trainings_participants WHERE PartStaffID=?";
        $sql="UPDATE ktv_msme_trainings_participants SET StatusCode = 'nullified',LastModifiedBy='".$_SESSION['userid']."',DateUpdated=NOW() WHERE PartStaffID = ? LIMIT 1";
        $stat = $this->db->query($sql, array($id));
        if ($stat) {
            $results['success'] = true;
            $results['message'] = "DELETED";
...
ORDER BY label";
    ...
    function check($training, $farmer) {
        $sql = "
            SELECT ParticipantStaffID as id FROM ktv_msme_trainings_participants WHERE MSMETrainingID=? and ParticipantPersonID=? AND StatusCode!='nullified'";
    ...
    function readParticipantsTraining($key) {
        $sql = "
            ...
            ,'-') AS jabatan
            FROM
                ktv_msme_trainings_participants a
                LEFT JOIN ktv_staffs s ON s.StaffID = a.PartStaffID
                LEFT JOIN ktv_persons b on s.PersonID=b.PersonID
                LEFT JOIN ktv_farmer kcf ON kcf.FarmerID = a.PartFarmerID
			...
			WHERE MSMETrainingID=? and IFNULL(e.StaffName, IFNULL(d.StaffName, b.PersonNm)) is not null and a.`StatusCode`!='nullified'
            ORDER BY PersonNm";
        $query = $this->db->query($sql, array($key));
        $result['data'] = $query->result_array();
        $query = $this->db->query('SELECT FOUND_ROWS() AS total');
    ...
    function readTraining($id) {
        $sql = "
            ...
            LEFT JOIN ktv_msme_trainings_sub_topics sub ON a.`MSMETrainingID` = sub.`MSMETrainingID`
...
LEFT JOIN ktv_persons b on s.PersonID=b.PersonID
LEFT JOIN ktv_farmer kcf ON kcf.FarmerID = a.PartFarmerID
LEFT JOIN ktv_msme_trainings_attendance bta ON bta.MSMETrainingID = a.`MSMETrainingID` AND ((PartType = 'staff' AND bta.AttStaffID = a.PartStaffID) OR (PartType = 'farmer' AND bta.AttFarmerID = a.PartFarmerID)) AND DayNumber = ?
WHERE
    a.MSMETrainingID = ?
GROUP BY a.MSMETrainingPartID
ORDER BY PartType, name
        ";
    ...
    public function updateFarmerAttendance($TrainingID, $type, $StaffID, $FarmerID, $DayNumber, $Attendance1, $Attendance2, $TrainingDate) {
        ...
        if($data['BANYAK'] > 0){
            //update
            return $this->db->update('ktv_msme_trainings_attendance',
                array(
            ...
            //insert
            $sql="INSERT INTO ktv_msme_trainings_attendance SET
                    MSMETrainingID   = ?,
                    AttType         = ?,
                    AttStaffID      = ?,
                    AttFarmerID     = ?,
