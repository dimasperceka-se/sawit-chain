...
class Supplychain_product extends REST_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('traceability_api/m_supplychain_product','_model');
    }
    ...
    public function fetch_get() {
        ...
        $data  = $this->_model->get_data( $this->get('SupplychainID'),$this->get('SupplychainProductID'), $this->recordStart, $this->recordLimit, $this->sortingField, $this->sortingDir);
        if($data){
    ...
    public function fetch_mill_get() {
        if($this->get('sort')) {
            $sorting      = json_decode($this->get('sort'));
            $this->sortingField = $sorting[0]->property;
            $this->sortingDir   = $sorting[0]->direction;
        ...
        $data  = $this->_model->get_mill_data( $this->get('SupplychainID'),$this->get('SupplychainProductID'), $this->recordStart, $this->recordLimit, $this->sortingField, $this->sortingDir);
        if($data){
            return $this->response(array('success' => true, 
                                         'message' => 'Data Berhasil Ditampilkan',
                                         'total' => $data['total'], 
    ...
    public function submit_mill_post(){
        ini_set('display_errors',true);
        error_reporting(E_ALL);
        $data = $this->post(NULL);
        if($data){
            $batch = $this->_model->submit_mill($data);
            if($batch){ 
                return $this->response($batch);
            }
        }else{
    ...
    public function product_get(){
        $output = $this->_model->product();

        $this->response($output, 200);
    }

    public function product_mill_get(){
        $output = $this->_model->mill_product();
