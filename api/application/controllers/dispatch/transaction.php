...
{ 
    public function __construct(){
        parent::__construct();
        $this->load->model('dispatch/mdispatch', '_model');
    }
	...
	public function fetchpick_get(){
        ...
        $data = $this->_model->fetchpick($this->get('DespatchID'), $this->get('start'),$this->get('limit'),$sortingField,$sortingDir, $this->get('ProductID'));
        if($data) $this->response($data, 200); else $this->response(array('error' => 'Couldn\'t find any datas!'), 404);
    }

    public function proses_batch_post(){
        $DespatchID = $_POST["DespatchID"];

        $output = array('success' => false, "message" => lang("Please Pick Process Batch"));
        $model = $this->_model->proses_batch($DespatchID);
        if($model) {
    ...
    public function sent_batch_post(){
        ...
        $model = $this->_model->sent_batch($DespatchID);
        if($model) {
            $this->response($model, 200);
        }
        $this->response($output, 400);
    ...
    public function fetchvehicle_get(){
        ...
        $data = $this->_model->fetchvehicle($this->get('DespatchID'), $this->get('start'),$this->get('limit'),$sortingField,$sortingDir, $this->get('ProductID'), $this->get('DespatchVehicleID'));
        if($data) $this->response($data, 200); else $this->response(array('error' => 'Couldn\'t find any datas!'), 404);
    }
    ...
    public function fetchvehiclebyID_get(){
        ...
        $data = $this->_model->fetchvehiclebyID($this->get('DespatchID'), $this->get('DespatchVehicleID'));
        if($data) $this->response($data, 200); else $this->response(array('error' => 'Couldn\'t find any datas!'), 404);
    }
    ...
    public function vehicle_list_post(){
        ...
        foreach ($data as $key => $value) {
            $keyNew = str_replace("Koltiva_view_Traceability_new_Dispatch_win_FormWinVehicle-Form-", '', $key);
            if($value == "") $value = null;
    ...
    public function information_grid_refinery_get(){
        ...
        $data = array("PKO"=>$_SESSION["PKO_REMAINING"],"CPO"=>$_SESSION["CPO_REMAINING"],"ProductPercentageCpo"=>$_SESSION["ProductPercentageCpo"], "ProductPercentagePk"=>$_SESSION["ProductPercentagePk"], "HaveOer"=>$_SESSION["HaveOer"], "flagCpo"=>$_SESSION["flagCpo"], "flagPk"=>$_SESSION["flagPk"]);
    ...
    public function del_vehicle_get(){
        $output = array('success' => true); 
		$DespatchVehicleID = $this->input->get('DespatchVehicleID'); 
        $model = $this->_model->del_vehicle($DespatchVehicleID);
        if($model) {
            $this->response($model, 200);
        }
        $this->response($output, 200);
