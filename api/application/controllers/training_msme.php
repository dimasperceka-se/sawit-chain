...
class Training_msme extends REST_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('training/Mmsme');
    }

    function datas_get() {
        $data = $this->Mmsme->readDatas($this->get('key'), $this->get('prov'), $this->get('dist'), $this->get('subdist'), $this->get('start'), $this->get('limit'), $_SESSION['userid']);
        if ($data)
    ...
    function data_get() {
        $data = $this->Mmsme->readData($this->get('id'));
        if ($data)
            $this->response($data, 200); // 200 being the HTTP response code
        else
            $this->response(array('error' => 'Couldn\'t find any units!'), 404);
    ...
    function data_delete() {
        if (!$this->delete('id'))
            $this->response(NULL, 400);
        $delete = $this->Mmsme->deleteData($this->delete('id'));
        if ($delete)
    ...
    function participants_get() {
        $data = $this->Mmsme->readParticipants($this->get('training'), $this->get('start'), $this->get('limit'));
        if ($data)
            $this->response($data, 200); // 200 being the HTTP response code
        else
            $this->response(array('error' => 'Couldn\'t find any units!'), 404);
    ...
    public function Kabupatens_get() {
        $data = $this->Mmsme->readKabupatens($this->get('ProvinceID'));
        if ($data) {
            $this->response($data, 200);
        } else {
            $this->response(array('error' => 'Couldn\'t find any programs!'), 404);
    ...
    public function Kecamatans_get() {
        $data = $this->Mmsme->readKecamatans($this->get('DistrictID'));
        if ($data) {
            $this->response($data, 200);
        } else {
    ...
    public function cpgs_get() {
        $data = $this->Mmsme->readCPGs($this->get('SubDistrictID'));
        if ($data) {
            $this->response($data, 200);
        }
    }
    ...
    function participants_add_farmer_get() {
        $data = $this->Mmsme->readParticipantsAddFarmer($this->get('training'), $this->get('ProvinceID'), $this->get('DistrictID'), $this->get('SubDistrictID'), $this->get('CPGid'), $this->get('key'), $this->get('start'), $this->get('limit'));
        if ($data)
            $this->response($data, 200); // 200 being the HTTP response code
        else
            $this->response(array('error' => 'Couldn\'t find any units!'), 404);
    ...
    function participants_add_staff_get() {
        $data = $this->Mmsme->readParticipantsAddStaff($this->get('training'), $this->get('role'), $this->get('key'), $this->get('start'), $this->get('limit'));
        if ($data)
            $this->response($data, 200); // 200 being the HTTP response code
        else
            $this->response(array('error' => 'Couldn\'t find any units!'), 404);
    ...
    function participants_post() {
        ...
        $result = $this->Mmsme->createParticipants(
    ...
    function data_participant_post() {
        if (!$this->post('training'))
            $this->response(NULL, 400);
        $add = $this->Mmsme->createParticipant($this->post('training'), $this->post('staf'), $this->post('wstart'), $this->post('wend'), $this->post('bstart'), $this->post('bend'), $_SESSION['userid']);
        if ($add)
            $this->response($add, 200); // 200 being the HTTP response code
        else
            $this->response(array('error' => 'Unit could not be found'), 404);
    ...
    function data_participant_put() {
        ...
        $update = $this->Mmsme->updateParticipant($this->put('wstart'), $this->put('wend'), $this->put('bstart'), $this->put('bend'), $this->put('id'), $_SESSION['userid']);
        if ($update)
            $this->response($update, 200); // 200 being the HTTP response code
        else
            $this->response(array('error' => 'Unit could not be found'), 404);
    ...
    function data_participant_delete() {
        if (!$this->delete('id'))
            $this->response(NULL, 400);
        $delete = $this->Mmsme->deleteParticipant($this->delete('id'));
        if ($delete)
            $this->response($delete, 200); // 200 being the HTTP response code
        else
            $this->response(array('error' => 'Unit could not be delete'), 404);
    ...
    function staffs_get() {
        $data = $this->Mmsme->readStaffs($this->get('query'));
        if ($data)
            $this->response($data, 200);
        else
    ...
    function check_get() {
        $data = $this->Mmsme->check($this->get('trainingid'), $this->get('staffid'));
        if ($data)
            $this->response($data, 200);
        else
    ...
    function cetak_get($id) {
        $data['data'] = $this->Mmsme->readTraining($id);
        if (empty($this->get('result'))) {
            $part = $this->Mmsme->readParticipantsTraining($id);
            $data['peserta'] = $part['data'];
        } else {
            $data['peserta'] = $this->Mmsme->getFarmerAttendanceDay($id, $this->get('DayNumber'));
        }
        $data['logo'] = $this->Mmsme->readPartnerLogo($id);
        $this->load->view('business_cetak_hadir', $data);
    }

    public function fasilitator_scpp_get()
    {
        $data = $this->Mmsme->getFacilitatorSCPP();
        $this->response($data, 200);
    }

    public function fasilitator_mitra_get()
    {
        $data = $this->Mmsme->getFacilitatorMitra();
        $this->response($data, 200);
    }
    ...
    public function participant_checklist_day_get() {
        ...
        $result = $this->Mmsme->getFarmerAttendanceDay($TrainingID, $DayNumber);
    ...
    public function attendance_day_post() {
        ...
        foreach ($data as $key => $value) {
            ...
            $result = $this->Mmsme->updateFarmerAttendance($TrainingID, $value['PartType'], $value['PartStaffID'], $value['PartFarmerID'], $DayNumber, $value['Attendance1'], $value['Attendance2'], $TrainingDate ? date('Y-m-d', strtotime($TrainingDate)) : null);
