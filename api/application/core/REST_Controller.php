...
abstract class REST_Controller extends CI_Controller
{
	...
	{
		/**
		 * Sebelum semua dijalankan, di validasi dulu yak ^_^
		 * 1. Validasi Session (Web Cocoatraceapp)
		 * 2. Validasi User-Agent mobile crosswalk
		...
		//$this->response(array('success' => false,'error' => 'Not Authorized! Please revoke token!'),401);
		...
		$httpRequestWith = isset($_SERVER['HTTP_X_REQUESTED_WITH']) ? $_SERVER['HTTP_X_REQUESTED_WITH'] : false;
		$serverIp        = $_SERVER['SERVER_ADDR'];

		$phpAuth = (string)isset($_SERVER['PHP_AUTH_USER']) ? $_SERVER['PHP_AUTH_USER'] : false;
		$phpPwd  = isset($_SERVER['PHP_AUTH_PW']) ? $_SERVER['PHP_AUTH_PW'] : '';
		$phpPwd  = (string)preg_replace("/[^a-zA-Z0-9.\-_\!]+/", "", $phpPwd);

		$userid = false;

		$url = $this->config->item('dhis_url') . 'api/me';
		...
		if (isset($_SESSION['userid']) && (int)$_SESSION['userid'] > 0) {
			$authorized = $this->_auth->getAuthSession((int)$_SESSION['userid']);
			$userid = $_SESSION['userid'];
		}

		//Khusus untuk app nya mas Rudi ya, sebelum boleh upload... harus check credential dulu ke dhis
		if(
			($class === 'common' && $method === 'upload') || 
			($class === 'farmer' && $method === 'mw_farmer_photo') || 
			($class === 'cpg' && $method === 'masterfarmergroups') || 
			($class === 'page' && $method === 'front_user_login') ||
			...
			if(
				...
				curl_setopt($ch, CURLOPT_HTTPHEADER, array(
					...
					'Authorization: Basic ' . base64_encode($phpAuth . ':' . $phpPwd)
				));
				...
				if (array_key_exists('id', $curlresult) && strlen($curlresult['id']) > 0) {
					$authorized = true;
				}
			}
		}

		// 2. Validasi User-agent mobile crosswalk, pengecekan kedua kalau ternyata url dipanggil bukan dari cocoatraceapp... misalnya dari Curl / Cron
		//    Ini ada batasannya ya, soalnya untuk Mobile Cocoatrace yang lama
		if (!$authorized) {

			//cara panggil lewat curl biar lewat tidak pakai pengecekan ini..
			//curl -A "mobile crosswalk" http://127.0.0.1/cocoatraceapi/index.php/system/units?_dc=1465386517400&page=1&start=0&limit=10

			if (strpos(strtolower($userAgent), 'mobile crosswalk') !== false) {
				$authorized = true;
			}
			...
			if (!$authorized) {
				...
				//second check, cek http_referernya
		...
		// contoh panggil di terminal pakai curl
		// curl --user admin:KoltivaAb2017! https://demo.cocoatrace.com/api/price/schedule
		if (!$authorized) {
			if (strlen($phpAuth) > 0 && strlen($phpPwd) > 0) {
				$authorized = $this->_auth->getAuthSession(false, $phpAuth, $phpPwd);
				if ($authorized == true) {
					//Cek PartnerID untuk di assign jika ada
					$PartnerID = $this->_auth->getSessionPartnerID($phpAuth);
					if ($PartnerID != "") $_SESSION['PartnerID'] = $PartnerID;
				}
			}
		}
		...
		if (!$authorized) {
			...
			$class = $this->router->fetch_class();
			$method = $this->router->fetch_method();
			//skip untuk sms inbound (sementara).
			if ($class == 'sms' && $method == 'inbound') {
				$authorized = true;
			}

			$reserved = array('mobile_auth', 'mobile_monitoring', 'sync');

			if (in_array($class, $reserved)) {
				$authorized = true; //$this->_getAuthToken();
			}
		}

		// 5. Khusus download ID Card, langsung by pass
		if (($class === 'farmer' && $method === 'print_id_card')) {
			$authorized = true;
		}
		...
		if (!$authorized) {
			...
			if ($authHeader) {
				...
				$parts = \explode('.', $token);
				if (3 !== \count($parts)) {
					$this->response(array('success' => false, 'message' => 'Format token is wrong !'));
				}

				// Cek validasi token
				$this->load->library('verifikasitoken');
				$cekValisasiToken = $this->verifikasitoken->cekValisasiToken($token, false);
				if ($cekValisasiToken['success']) {
					$authorized = true;
				} else {
					$this->response(array('success' => false, 'message' => $cekValisasiToken['message']));
				}
			}
		}
		...
		//check dulu ke database, apakah grant akses ke fungsi itu?
	...
	public function __construct()
	{
		...
		//Set Language
		if (!in_array($this->router->fetch_class(), array('tools'))) {
			load_lang();
		}
	}
	...
	/**
	 * Validasi awal untuk mobile dengan memeriksa token yang disertakan pada header
	 *
	...
	private function _getAuthToken()
	{
		...
		$user = $this->encrypt->decode($token, 'bismillah');
		$this->user = $user;
		$this->district = $this->_auth->getUserDistrict($user);
		if ($this->_auth->checkToken($token, $user)) {
			return true;
		}
