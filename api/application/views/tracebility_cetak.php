...
<div class="page">
  ...
  <table width="100%" cellspacing="4" class="header">
   ...
   <tr><th>No</th><th>Batch</th><th>Bruto</th><th>Netto</th></tr>
   <?for ($i=0;$i<sizeof($detail);$i++) {?>
      <tr><td rowspan="2"><?=$i+1?></td><td><?=$detail[$i]['SupplyBatchNumber']?></td>
         <td align="right"><?=number_format($detail[$i]['VolumeBruto'],2,',','.')?></td>
         <td align="right"><?=number_format($detail[$i]['VolumeNetto'],2,',','.')?></td></tr>
      <tr><td colspan="3">
         <?
         $det = explode('|',$detail[$i]['trans']);
         if (sizeof($det)>0) {?>
            <table width="100%" cellspacing="2" class="body">
            <tr><th>No</th><th>Farmer</th><th>Jenis</th><th>Bruto</th><th>Netto</th><th>Nett Price</th><th>Total Harga</th></tr>
            <?for ($j=0;$j<sizeof($det);$j++) {
               $de = explode('#',$det[$j]);?>
               <tr><td><?=$j+1?></td><td><?="[$de[0]] $de[1]"?></td><td><?=$de[2]>0?'FF':'FAQ'?></td>
                  <td align="right"><?=$de[2]>0?$de[2]:$de[3]?></td><td align="right"><?=$de[4]>0?$de[4]:$de[5]?></td>
                  <td align="right"><?=$de[6]>0?$de[6]:$de[7]?></td><td align="right"><?=$de[7]>0?$de[7]:$de[8]?></td></tr>
            <?}?>
            </table>
         <?}?>
      </td></tr>
