...
<div class="page">
    ...
    <tr>
       ...
       <td id="mainColumn" valign="top">
          ...
          <tr>
             <td width="80%" id="tdMainTitle" style="border-right: 1px dotted #95130A;">
          ...
          <table width="100%" style="margin-bottom: 8px;margin-top:-8px;">
             <tr>
                ...
                <td class="tdTitleMain" valign="top">
                   <?php echo strtoupper(lang('Training / Coaching Participation'))?>
                </td>
             </tr>
          </table>
          ...
          <table class="tabelListData" width="100%" border="0">
             <tr>
                ...
                <th><?php echo strtoupper(lang('Training / Coaching'))?></th>
                ...
                if (count($trainings) > 0) {
                    foreach ($trainings as $key => $training) {
                        ...
                        if($training['type'] == "training") {
                            if ($training['CpgTrainings'] != "") {
                                if ($training['sub_topic'] != "") {
                                    $train .= $training['CpgAbbre'];
                                } else {
                                    $train .= $training['CpgTrainings'];
                                }

                                if ($training['sub_topic']) {
                                    $train .= ' - [' . $training['sub_topic'] . ']';
                                }

                                $train = lang('Training').' - ['.$train.']';
                            } else {
                                $hidden = 'yes';
                            }
                        }

                        if($training['type'] == "coaching") {
                            if(!empty($training['Topic'])) {
                                $train = lang('Coaching').' - ['.$training['Topic'].']';
                            } else {
                                $hidden = 'yes';
                            }
                        }

                        if($training['TrainingStart'] != "-") $TrainingStart = date('d M Y', strtotime($training['TrainingStart'])); else $TrainingStart = "-";
                        if($training['TrainingEnd'] != "-") $TrainingEnd = date('d M Y', strtotime($training['TrainingEnd'])); else $TrainingEnd = "-";
                    ...
                    echo '
                        <tr>
                            <td align="center" colspan="5"> - '. lang('No Data') .' - </td>
                        </tr>';
                }
             ?>
          </table>
    ...
    <div id="mainContainerPolygon" style="background-image: none !important;">
        ...
        <script type="text/javascript">
            function initMapPolygon<?php echo $member['MemberID'] ?>() {
                let areasNew = <?php echo json_encode(array($areaPolygon)) ?>;
                let zoomPolygon = 17

                if (areasNew.length > 1) {
                    zoomPolygon = 16
                } else {
                    if (areasNew[0].length > 3) {
                        zoomPolygon = 14
                    } else {
                        zoomPolygon = 16
                    }
                ...
                function show_polygon_new() {
                    ...
                    for(let i = 0; i < areasNew.length; i++){
                        let areaPolygon = areasNew[i];

                        for (let j = 0; j < areaPolygon.length; j++) {
                            let valuenya = areaPolygon[j]
                    ...
                    if (count($polygon_list) > 0) {
                        ...
                        foreach ($polygon_list as $key => $value) {
                            echo '<tr>
                                  ...
                                  <td align="center">' . number_format($value['PolygonSize'], 2, ".", ",") . '</td>
                                  </tr>';
                            $increList++;
                        }
                    } else {
