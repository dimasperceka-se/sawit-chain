...
<html lang="en">
    ...
    <body>
            <div class="row-fluid">
                <div class="span12">
                    <div id="map_canvas_idtrader" class="gmap3" style="width: 100%; min-height:460px"></div>
                </div>
            </div>
            <input  style="display:none;float:left; height:37px;margin-right: 5px;" type="text" class="" id="luas_idtrader" placeholder="Area (Ha)" size="10" value="<?php echo $luas?>" readonly>
            <button style="display:none" type="button" class="btn" data-toggle="button" id="button-add-marker_idtrader">Add Marker</button>
            <button style="display:none; color: #fff;background-color: #d9534f;border-color: #d43f3a;" type="button" class="btn btn-danger" id="clear-marker_idtrader">Clear All Marker</button>
            <button style="display:none; color: #fff;background-color: #f0ad4e;border-color: #eea236;"                     type="button" class="btn" id="delete-marker_idtrader"  data-id="" ></button>
            <button style="display:none; color: #fff;background-color: #337ab7;border-color: #2e6da4;"  type="button" class="btn" id="save-polygon_idtrader">Save Polygon</button>
            <button style="display:none; color: #fff;background-color: #403b3b;border-color: #3a3636;" type="button" class="btn" id="cancel-polygon_idtrader">Cancel</button>
            <button type="button" style="<?php echo $hakAksesPolygonStyle?>" class="btn btn-primary" id="edit-polygon_idtrader">Edit Polygon</button>
            ...
            <script type="text/javascript">
                ...
                function send_data () {
                    map = $('#map_canvas_idtrader').gmap3('get');
                    var center = map.getCenter();
                    var lati =  center.lat();
                    var langi = center.lng();
                    map = $("#map_canvas_idtrader").gmap3({get: {name: "map"}});
                    fixMap(map);
                    $.ajax({
                        ...
                        marker: {
                            ...
                            events: {
                                ...
                                click: function(marker, event, data) {
                                    var button = $('#delete-marker_idtrader');
                ...
                function makePoly()
                {
                    ...
                    $('#markers-text_idtrader').html('');
                    $.each(markers, function(i){
                        if (markers[i]) {
                            tmp_marker.push([markers[i].lat, markers[i].lng]);
                            $('#markers-text_idtrader').append('['+markers[i].lat + ',' + markers[i].lng + '],<br/>');
                        }
                    });
                    $("#map_canvas_idtrader").gmap3({
                        clear: {
                ...
                function show_polygon () {
                    ...
                    $('#map_canvas_idtrader').gmap3("get").fitBounds(area_bounds);
                ...
                function setMapSize() {
                    ...
                    // $('#map_canvas_idtrader').css('width', width * (90 / 100));
                    // $('#map_canvas_idtrader').css('height', height * (80 / 100));
                }
                ...
                $(function() {
                            ...
                            events: {
                                click: function(map, event, data){
                                    if (mode == 'add') {
                                        ...
                                        // console.log(data);
                                        // $('#button-add-marker_idtrader').click();
                                    }
                                }
                            }
                        }
                    ...
                    //$('#add-marker').click(function() {
                        //addMarker();
                    //});
                    $('#button-add-marker_idtrader').click(function() {
                        // switch mode
                        mode = (mode == 'normal')?'add':'normal';
                        var map_hidden_div = $('#map_canvas_idtrader > div').children('div:first').children('div');
                        if (mode == 'add') {
                    ...
                    $('#delete-marker_idtrader').click(function() {
                        var id = $(this).data('id');
                        clearMarker(id);
                        markers.splice(id, 1);
                        $(this).hide();
                    });
                    $('#clear-marker_idtrader').click(function() {
                        clearMarker();
                        markers.splice(0, markers.length);
                        makePoly();
                        $('#delete-marker_idtrader').hide();
                        $('#luas_idtrader').val('');
                    });

                    $('#save-polygon_idtrader').click(function() {
                        ...
                        if(luas_area==0.00){
                            ...
                            hide_polygon();
                            show_polygon();
                            $('#button-add-marker_idtrader').hide();
                            $('#clear-marker_idtrader').hide();
                            $('#save-polygon_idtrader').hide();
                            $('#cancel-polygon_idtrader').hide();
                            $('#luas_idtrader').hide();
                            $('#edit-polygon_idtrader').show();
                            mode = 'normal';
                            var map_hidden_div = $('#map_canvas_idtrader > div').children('div:first').children('div');
                            $('#button-add-marker_idtrader').removeClass('btn-primary');
                            map_hidden_div.css('cursor', 'url("http://maps.gstatic.com/mapfiles/openhand_8_8.cur"), default');
                            $('#cLosePolygon_idtrader').click();
                        }

                    });

                    $('#cancel-polygon_idtrader').click(function() {
                        clearMarker();
                        markers.splice(0, markers.length);
                        makePoly();
                        $('#delete-marker_idtrader').hide();
                    ...
                    $('#edit-polygon_idtrader').click(function() {
                        $('#button-add-marker_idtrader').show();
                        $('#clear-marker_idtrader').show();
                        $('#save-polygon_idtrader').show();
                        $('#cancel-polygon_idtrader').show();
                        $('#luas_idtrader').show();
                        $('#edit-polygon_idtrader').hide();
                        edit_polygon();
                        mode = 'normal';
                        var map_hidden_div = $('#map_canvas_idtrader > div').children('div:first').children('div');
                        $('#button-add-marker_idtrader').removeClass('btn-primary');
