...
<div class="page">
  ...
  <table width="100%" cellspacing="4" class="header">
   ...
   <tr><th>No</th><th>Batch</th><th>Nama Petani</th><th>Tanggal</th><th>Bruto</th><th>Moisture</th><th>Price</th><th>Payment</th>
      <!--<th>Netto</th>--></tr>
   <?for ($i=0;$i<sizeof($detail);$i++) {
      if ($detail[$i]['SupplyBatchNumber']!='') {?>
      <tr><td><?=$i+1?></td>
         <td align="right"><?=$detail[$i]['SupplyBatchNumber']?></td>
         <td><?='['.$detail[$i]['FarmerID'].'] '.$detail[$i]['FarmerName']?></td>
         <td><?=SetTanggal($detail[$i]['DateTransaction'])?></td>
         <td align="right"><?=$detail[$i]['FAQVolumeBruto']?></td>
         <td align="right"><?=$detail[$i]['Moisture']?></td>
         <td align="right"><?=$detail[$i]['FAQNetPrice']?></td>
         <td align="right"><?=$detail[$i]['FAQTotalPayment']?></td>
         <!--<td align="right"><?=round($detail[$i]['FAQTotalPayment']/$detail['total'][$detail[$i]['SupplyBatchNumber']]*$detail[$i]['DestWeight'],2)?></td>
         <td align="right"><?=round((100-($detail[$i]['Moisture']>$detail[$i]['FAQMoisture']?$detail[$i]['Moisture']-$detail[$i]['FAQMoisture']:0))/100*
            $detail[$i]['FAQVolumeBruto'],2)?></td>-->
      </tr>
   <?}}?>
   </table>
